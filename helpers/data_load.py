...
class cifar10_dataset(Data.Dataset):
    def __init__(self, train=True, transform=None, target_transform=None, split_per=0.9, random_seed=1,
                 length_train_data=20000, args=None, logger=None):
        ...
        self.train_data, self.val_data, self.train_labels, self.val_labels, train_set_index = helpers.tools.dataset_split(
            original_images, original_labels, split_per, random_seed, num_class)
        ...
        if self.train:
            if args.annotator_type == 'synthetic':
                ...
                self.annotations_one_hot, self.annotations, self.annotations_list_maxmig, \
                    self.annotator_softmax_label_mbem, self.annotators_per_sample_mbem, self.annotator_mask = generate_annotator_labels(
                    self.A_true, args.annotator_label_pattern, args.p, args.l, self.train_labels)
                ...
                self.annotations_one_hot, self.annotations, self.annotations_list_maxmig, \
                    self.annotator_softmax_label_mbem, self.annotators_per_sample_mbem, self.annotator_mask = get_real_annotator_labels(
                    annotations, args.K)
    ...
    def __getitem__(self, index):
        ...
        if self.train:
            img, annot, annot_one_hot, annot_mask, annot_list, label = self.train_data[index], self.annotations[index], \
            self.annotations_one_hot[index], self.annotator_mask[index], self.annotations_list_maxmig[index], \
...
class mnist_dataset(Data.Dataset):
    def __init__(self, train=True, transform=None, target_transform=None, split_per=0.9, random_seed=1,
                 length_data=20000, args=None, logger=None):
        ...
        if args.annotator_type == 'machine-classifier':
            ...
            data_train_annotators, train_val_data, y_train_annotators, train_val_labels = train_test_split(
                original_images, original_labels, train_size=10000 / length_original_data)
            print('shape of images for training annotators')
            print(data_train_annotators.shape)

            # Getting training and validation data
            train_val_data_selected, _, train_val_labels_selected, _ = train_test_split(train_val_data,
                                                                                        train_val_labels,
                                                                                        train_size=(length_data) / (
                                                                                                    length_original_data - 10000))

            logger.info('Splitting train and validation data')
            self.train_data, self.val_data, self.train_labels, self.val_labels, train_set_index = helpers.tools.dataset_split(
                train_val_data_selected, train_val_labels_selected, split_per, random_seed, num_class)
            ...
            if args.annotator_type == 'synthetic':
                ...
                self.annotations_one_hot, self.annotations, self.annotations_list_maxmig, \
                    ...
                    = generate_machine_classifier_annotations(data_train_annotators, \
                                                              y_train_annotators, self.train_data, self.train_labels,
                                                              args, logger, self.transform, self.target_transform)
...
class music_dataset(Data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.train:
            return img.astype(np.float32), annot, annot_one_hot, annot_mask, annot_list, label
        else:
...
class music_test_dataset(Data.Dataset):
    def __init__(self, transform=None, target_transform=None, args=None, mean_train=None, std_train=None,
                 label_list=None):
